...
from .attention import WanSelfAttention
...
class DiTBlock(nn.Module):
    ...
    def forward(
        ...
        if x_ip is not None:
            ...
            self.self_attn.cond_group_sizes = condition_token_counts
            input_x = torch.concat([input_x, input_x_ip], dim=1)
            self.self_attn.kv_cache = None
        else:
            self.self_attn.cond_size = None
            self.self_attn.cond_group_sizes = None
